export const onRequestPost: PagesFunction = async (context) => {
  ...
  const extras = ['"wish list"','wishlist','"gift registry"','"wedding registry"','"baby registry"']
  const q = `${quoted} (${sites.map(s=>`site:${s}`).join(' OR ')} OR ${extras.join(' OR ')}) ${location||''}`
  const url = new URL('https://www.googleapis.com/customsearch/v1')
  url.searchParams.set('key', key); url.searchParams.set('cx', cx); url.searchParams.set('q', q); url.searchParams.set('num','7')
